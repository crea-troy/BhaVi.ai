...
class FieldWaveSystem(nn.Module):
    ...
    def encode_text(self, text: str) -> torch.Tensor:
        ...
        pos    = torch.linspace(0, math.pi, len(ids))        # [L]
        # pos: [L], freqs: [H, D] → broadcast over sequence
        harm   = torch.sin(
            pos.unsqueeze(1).unsqueeze(2) * self.freqs.unsqueeze(0) + self.phases.unsqueeze(0)
        ).mean(1)                                            # [L, D]
